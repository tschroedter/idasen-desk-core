    ...
    : IGattCharacteristicValueChangedObservables
{
    ...
    public async Task Initialise ( GattCharacteristic characteristic )
    {
        ...
        _logger.Information (
                             "Service UUID = {ServiceUuid} Characteristic UUID = {CharacteristicUuid} Notify = {Notify} Indicate = {Indicate} Write = {Write} WriteWithoutResponse = {WriteWithoutResponse}" ,
                             serviceUuid ,
                             characteristic.Uuid ,
        ...
        if ( properties.HasFlag ( GattCharacteristicProperties.Notify ) ||
            ...
            try {
                ...
                if ( status == GattCommunicationStatus.Success ) {
                    _logger.Information (
                                         "Notify/Indicate Service UUID = {ServiceUuid} Characteristic UUID = {CharacteristicUuid} - Subscribing to ValueChanged" ,
                                         serviceUuid ,
                                         characteristic.Uuid ) ;
        ...
        ISubject < GattCharacteristicValueChangedDetails > subject )
    {
        try {
            ...
            var details = new GattCharacteristicValueChangedDetails (
                                                                     sender.Uuid ,
                                                                     bytes ,
                                                                     args.Timestamp ) ;
