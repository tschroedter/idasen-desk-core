    ...
    : IGattServicesProvider
{
    ...
    public async Task Refresh()
    {
        ...
        if (_gattResult.Status == GattCommunicationStatus.Success)
            ...
            _logger.Error(
                "[{DeviceId}] Gatt communication status '{Status}'",
                _device.DeviceId,
                _gattResult.Status);
        }
    ...
    private async Task GetCharacteristicsAsync(IGattDeviceServicesResultWrapper gatt)
    {
        foreach (var service in gatt.Services) {
            ...
            if (characteristics.Status != GattCommunicationStatus.Success) {
                _logger.Error(
                    "[{DeviceId}] Could not get Characteristics for device '{ServiceDeviceId}' and service '{ServiceUuid}'",
                    _device.DeviceId,
                    service.DeviceId,
                    service.Uuid);
