# TestLogger - Serilog Test Helper

## Overview

`TestLogger` is a reusable test utility for capturing and asserting Serilog log output in unit tests. It provides a real `ILogger` instance that writes to an in-memory buffer, allowing you to verify logging behavior without using mocks.
...
```
src/Idasen.BluetoothLE.Characteristics.Tests/Common/TestLogger.cs
```
...
{
    // Arrange
    using var testLogger = new TestLogger();
    var sut = new YourClass(testLogger.Logger);
...
public void Test_OnlyCaptureWarningsAndErrors()
{
    ...
    using var testLogger = new TestLogger(LogEventLevel.Warning);
    var sut = new YourClass(testLogger.Logger);
    ...
    // Assert
    testLogger.Contains("Error occurred")
              .Should()
              .BeTrue();

    // Debug messages won't be captured
    testLogger.Contains("Debug message")
              .Should()
              .BeFalse();
}
```
...
public void Test_ExactLogMessage()
{
    ...
    // Assert
    testLogger.Contains("processing started")        // Case-insensitive
              .Should()
              .BeTrue();

    testLogger.ContainsExact("Processing STARTED")   // Case-sensitive
              .Should()
              .BeTrue();
}
```
...
public void Test_MultipleOperations()
{
    ...
    // Act - First operation
    sut.OperationOne();
    testLogger.Contains("Operation One complete").Should().BeTrue();
    ...
    testLogger.Contains("Operation One").Should().BeFalse();
    testLogger.Contains("Operation Two complete").Should().BeTrue();
}
```
...
```csharp
public TestLogger(LogEventLevel minimumLevel = LogEventLevel.Verbose)
```
...
public void Initialize()
{
    ...
    _sut = new DeskMover(_testLogger.Logger, ...);
}
...
public void Test_Something()
{
    ...
    _testLogger.Contains("Expected log message")
               .Should()
               .BeTrue();
}
```
...
See `TestLoggerExamples.cs` in the same directory for complete working examples.

## Notes

- Always use `using` or dispose the TestLogger to clean up resources
- The logger captures formatted log messages (template resolved with values)
- Log format is: `[Level] Message - ExceptionMessage`
- The TestLogger is thread-safe for writing (Serilog guarantees this)
