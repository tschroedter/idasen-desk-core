...
public class RawValueHandlerTests
{
    ...
    public void Initialize ( )
    {
        ...
        _rawValueHandler = new RawValueHandler ( _valueReader ,
                                                 _valueWriter ) ;
    }
    ...
    public async Task TryReadValueAsync_ShouldCallValueReader ( )
    {
        ...
        _valueReader.TryReadValueAsync ( characteristic ).Returns ( Task.FromResult ( ( true , new byte [ ]
                                                                                               {
                                                                                                   1 ,
                                                                                                   2 ,
                                                                                                   3
        ...
        result.Item2.Should ( ).BeEquivalentTo ( new byte [ ]
                                                 {
                                                     1 ,
                                                     2 ,
                                                     3
    ...
    public async Task TryWriteValueAsync_ShouldCallValueWriter ( )
    {
        ...
        _valueWriter.TryWriteValueAsync ( characteristic ,
                                          buffer ).Returns ( Task.FromResult ( true ) ) ;

        var result = await _rawValueHandler.TryWriteValueAsync ( characteristic ,
                                                                 buffer ) ;

        result.Should ( ).BeTrue ( ) ;
        await _valueWriter.Received ( 1 ).TryWriteValueAsync ( characteristic ,
                                                               buffer ) ;
    }
    ...
    public async Task TryWritableAuxiliariesValueAsync_ShouldCallValueWriter ( )
    {
        var characteristic = Substitute.For < IGattCharacteristicWrapper > ( ) ;
        var buffer         = Substitute.For < IBuffer > ( ) ;
        _valueWriter.TryWritableAuxiliariesValueAsync ( characteristic ,
                                                        buffer ).Returns ( Task.FromResult ( true ) ) ;

        var result = await _rawValueHandler.TryWritableAuxiliariesValueAsync ( characteristic ,
                                                                               buffer ) ;

        result.Should ( ).BeTrue ( ) ;
        await _valueWriter.Received ( 1 ).TryWritableAuxiliariesValueAsync ( characteristic ,
                                                                             buffer ) ;
    }
    ...
    public async Task TryWriteWithoutResponseAsync_ShouldCallValueWriter ( )
    {
        ...
        _valueWriter.TryWriteWithoutResponseAsync ( characteristic ,
                                                    buffer ).Returns ( Task.FromResult ( writeResult ) ) ;

        var result = await _rawValueHandler.TryWriteWithoutResponseAsync ( characteristic ,
                                                                           buffer ) ;

        result.Should ( ).Be ( writeResult ) ;
        await _valueWriter.Received ( 1 ).TryWriteWithoutResponseAsync ( characteristic ,
                                                                         buffer ) ;
    }
}